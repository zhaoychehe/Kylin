...
-(void)launchForKey:(NSString *)key{
    ...
    DTExcuteFunctionsForKey([newKey UTF8String],
                            ^(char * name){
        [self->_logger willExcuteFunction:[NSString stringWithUTF8String:name] forLaunchKey:key];
    },^(char * name){
        [self->_logger didExcutedFunction:[NSString stringWithUTF8String:name] forLaunchKey:key];
    });
